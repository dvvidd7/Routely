...
export default function RecentSearch({ searchRef, userSearch, onPress }: SearchRefType) {
    ...
    const handlePress = () => {
        if (searchRef && searchRef.current && userSearch) {
        ...
        else return console.warn("NO SEARCH FOUND");
    };
    ...
    return (
        <TouchableOpacity onPress={handlePress} style={{...styles.container,backgroundColor: dark ? 'black' : 'white',}}>
            ...
            <Text style={{...styles.text, color: dark ? 'white' : 'black'}}>{userSearch.searchText}</Text>
        </TouchableOpacity>
    );
}
